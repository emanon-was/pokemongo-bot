...
(defprotocol IPokemonId
  (pokemon-id [_]))

(extend-protocol IPokemonId
  PokemonIdOuterClass$PokemonId
  (pokemon-id [pk]
    (let [descriptor (.getValueDescriptor pk)]
      {:name   (.getName descriptor)
       :number (.getNumber descriptor)
       :jname  (*pokemons* (.getNumber descriptor))}))
  Pokemon
  (pokemon-id [pk]
    (pokemon-id (.getPokemonId pk)))
  CatchablePokemon
  (pokemon-id [pk]
    (pokemon-id (.getPokemonId pk))))

(defprotocol PokemonStatus
  (status [_]))

(extend-protocol PokemonStatus
  Pokemon
  (status [pk]
    {:pokemon-id (pokemon-id pk)
     :cp (.getCp pk)
     :cp-multiplier (.getCpMultiplier pk)
